 ...
 */
void Keys::RegisterAction(int key, int modifiers, std::function<void(float)> action) {
    // 将新的绑定添加到动作列表
 ...
 */
void Keys::ProcessInput(GLFWwindow* window, float velocity) {
    // 遍历所有已注册的动作绑定
 ...
 */
bool Keys::CheckModifiers(GLFWwindow* window, int requiredMods) {
    // 逻辑解释：
...
bool Keys::IsShiftPressed(GLFWwindow* window) {
    return glfwGetKey(window, GLFW_KEY_LEFT_SHIFT)  == GLFW_PRESS ||
           glfwGetKey(window, GLFW_KEY_RIGHT_SHIFT) == GLFW_PRESS;
}

bool Keys::IsControlPressed(GLFWwindow* window) {
    return glfwGetKey(window, GLFW_KEY_LEFT_CONTROL)  == GLFW_PRESS ||
           glfwGetKey(window, GLFW_KEY_RIGHT_CONTROL) == GLFW_PRESS;
}
// 点击Alt键
bool Keys::IsAltPressed(GLFWwindow* window) {
    return glfwGetKey(window, GLFW_KEY_LEFT_ALT)  == GLFW_PRESS ||
           glfwGetKey(window, GLFW_KEY_RIGHT_ALT) == GLFW_PRESS;
}

bool Keys::IsSuperPressed(GLFWwindow* window) {
...
// 初始化默认输入绑定
/**
 * @param keys 要初始化的Keys实例
 * @param window GLFW窗口指针
 * @param camera 相机对象指针（用于移动控制）
 * @note 演示了多种绑定方式：基础按键、组合键、lambda表达式使用
 */
void Keys::Init(Keys& keys, GLFWwindow* window, Camera* camera) {
    // 注册ESC退出（无修饰键）
    keys.RegisterAction(GLFW_KEY_ESCAPE, 0, [window](float) {
    ...
    auto registerMovement = [&](int key, auto direction) {
        keys.RegisterAction(key, 0, [camera, direction](float velocity) {
            // 根据方向函数获取移动方向，乘以速度系数
    ...
    // 垂直移动绑定
    keys.RegisterAction(GLFW_KEY_SPACE, 0, [camera](float velocity) {
        camera->Position.y += velocity;  // 空格键上升
    });
    keys.RegisterAction(GLFW_KEY_LEFT_SHIFT, 0, [camera](float velocity) {
        camera->Position.y -= velocity;  // Shift键下降
    });

    // 组合键示例：Ctrl+W加速前进（3倍速度）
    keys.RegisterAction(GLFW_KEY_W, GLFW_MOD_CONTROL, [camera](float velocity) {
