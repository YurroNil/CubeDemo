// src/core/windowMng.cpp
#include "utils/streams.h"
#include "core/windowMng.h"
#include "core/inputHandler.h"
namespace CubeDemo {

void WindowMng::Init(int width, int height, const char* title) {
    // 初始化窗口
    ...
    glfwSetCursorPosCallback(s_Window, [](GLFWwindow* w, double x, double y) { InputHandler::MouseCallback(x, y); });
    glfwSetScrollCallback(s_Window, [](GLFWwindow* w, double x, double y) { InputHandler::ScrollCallback(y); });
...
bool WindowMng::ShouldClose() {
    return glfwWindowShouldClose(s_Window);
}

void WindowMng::ToggleFullscreen(GLFWwindow* window) {
...
void WindowMng::FullscreenTrigger(GLFWwindow* window) {
...
void WindowMng::UpdateWindowSize(GLFWwindow* window) {
    // 更新成员的值
    glfwGetWindowSize(window, &s_WindowWidth, &s_WindowHeight);
}

void WindowMng::UpdateWindowPos(GLFWwindow* window) {
    // 更新成员的值
