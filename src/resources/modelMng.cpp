// src/graphics/modelMng.cpp
#include "resources/modelMng.h"
#include "utils/streams.h"

namespace CubeDemo {

void ModelMng::Register(const string& name, 
...
// 渲染循环
void ModelMng::Render(const string& name, const Camera& camera, float aspectRatio) {
...
// 删除模型
void ModelMng::Delete(const string& name) {
    if (s_Models.find(name) != s_Models.end()) {
        auto& instance = s_Models[name];
...
void ModelMng::SetPosition(const string& name, const vec3& position) {
    if(s_Models.find(name) != s_Models.end()) {
        s_Models[name].transform.position = position;
    }
}

void ModelMng::SetRotation(const string& name, const vec3& rotation) {
    if(s_Models.find(name) != s_Models.end()) {
        s_Models[name].transform.rotation = rotation;
    }
}

void ModelMng::SetScale(const string& name, const vec3& scale) {
    if(s_Models.find(name) != s_Models.end()) {
        s_Models[name].transform.scale = scale;
    }
}

void ModelMng::Move(const string& name, const vec3& delta) {
    if(s_Models.find(name) != s_Models.end()) {
        s_Models[name].transform.position += delta;
    }
}

void ModelMng::Rotate(const string& name, const vec3& delta) {
    if(s_Models.find(name) != s_Models.end()) {
        s_Models[name].transform.rotation += delta;
    }
}

void ModelMng::Scale(const string& name, const vec3& delta) {
    if(s_Models.find(name) != s_Models.end()) {
        s_Models[name].transform.scale += delta;
    }
}

mat4 ModelMng::GetModelMatrix(const string& name) {
    if(s_Models.find(name) != s_Models.end()) {
