...
void UIMng::RenderLoop(GLFWwindow* window, Camera* camera) {
    ...
    if(MainMenuPanel::s_isMainMenuPhase) return;

    if (INPUTS::s_isGamePaused) PausePanel::Render(window);

    if (INPUTS::s_isDebugVsble) {
        ControlPanel::Render(camera); // 控制面板
        DebugPanel::Render(camera);   // 调试面板
    }

    // 编辑模式面板
    if (INPUTS::s_isEditMode) {
        EditPanel::Render(camera);
    }  else if(CMTP::s_FBO_Inited) CMTP::Cleanup();

    // 预设库面板
    if (INPUTS::s_isPresetVsble) PresetlibPanel::Render(camera);
}
...
void UIMng::ConfigureImGuiStyle() {
    ...
    colors[ImGuiCol_TabDimmedSelected] = ImVec4(0.20f, 0.20f, 0.20f, 0.60f);
